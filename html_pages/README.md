...
python3 html_pages/show_local_branches.py \
  --repo-path ~/dev \
  --output ~/dev/speedoflight/dynamo/users/keivenchang/local.html \
...
**Output locations:**
- `$HOME/dev/speedoflight/users/kthui/index.html`
- `$HOME/dev/speedoflight/users/keivenchang/index.html`

**Logs:**
- Working hours: `~/dev/logs/YYYY-MM-DD/remote_prs_working.log`
- Off hours: `~/dev/logs/YYYY-MM-DD/remote_prs_offhours.log`
...
- Per-run logs: `~/dev/logs/YYYY-MM-DD/<job>.log`
...
ERROR: Failed to update /home/keivenc/dev/speedoflight/dynamo/users/keivenchang/local.html
See log for details: /home/keivenc/dev/logs/2026-01-23/show_local_branches.log
```

1. Check the log file path shown in the error message
2. Look for Python tracebacks or error messages:
   ```bash
   tail -50 /home/keivenc/dev/logs/2026-01-23/show_local_branches.log
   ```
...
**Notes:**
- If running from cron, expect most output to go to files under `~/dev/logs/<YYYY-MM-DD>/` (not stdout)
...
```bash
tail -n 200 ~/dev/logs/$(date +%Y-%m-%d)/cron.log
tail -n 200 ~/dev/logs/$(date +%Y-%m-%d)/show_commit_history.log
tail -n 200 ~/dev/logs/$(date +%Y-%m-%d)/show_local_branches.log
```
