...
from common_build_report import BuildReport
    ...
    def _generate_commit_history_html(
        ...
        for commit in commit_data:
            ...
            if matching_logs_for_composite:
                ...
                for commit_sha in composite_to_commits[composite_sha]:
                    json_pattern = str(log_dir / f"*.{commit_sha}.json")
        ...
        for commit in commit_data:
            ...
            # The status indicator reflects LOCAL builds only (read from BuildReport JSON)
        ...
        for sha_short_key, log_entries in log_paths.items():
            ...
            json_abs = html_abs.parent / html_abs.name.replace(".report.html", ".json")
            ...
            try:
                ...
                for img in sorted(imgs, key=lambda x: x.tag):
                    size_display = "N/A"
                    if img.size_bytes and img.size_bytes > 0:
                        if img.size_bytes >= 1_000_000_000:
                            size_display = f"{img.size_bytes / 1_000_000_000:.1f}GB"
                        elif img.size_bytes >= 1_000_000:
                            size_display = f"{img.size_bytes / 1_000_000:.1f}MB"
                        else:
                            size_display = f"{img.size_bytes / 1_000:.1f}KB"
                    created_display = "N/A"
                    if img.pushed_at:
                        try:
                            dt = datetime.fromisoformat(img.pushed_at.replace('Z', '+00:00'))
                            dt = dt.astimezone(ZoneInfo('America/Los_Angeles'))
                            created_display = dt.strftime('%Y-%m-%d %H:%M')
                        except (ValueError, TypeError):
                            created_display = img.pushed_at[:16] if len(img.pushed_at) >= 16 else img.pushed_at
                    elif report.report_generated:
                        # Fallback: use report generation time as approximate push time
                        try:
                            dt = datetime.fromisoformat(report.report_generated.replace('Z', '+00:00'))
                            dt = dt.astimezone(ZoneInfo('America/Los_Angeles'))
                            created_display = dt.strftime('%Y-%m-%d %H:%M')
                        except (ValueError, TypeError):
