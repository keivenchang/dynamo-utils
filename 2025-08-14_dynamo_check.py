...
├─ Python: 3.12.3 (/opt/dynamo/venv/bin/python3); PYTHONPATH=:~/dynamo/components/planner/src:
...
class NVIDIAGPUDetector:
    ...
    def get_gpu_info(self) -> Tuple[List[str], Optional[str], Optional[str]]:
        ...
        if not names:
            ...
            return [f"❌ NVIDIA GPU: not detected (driver {driver}, CUDA {cuda})"], driver, cuda
...
class DynamoChecker:
    ...
    def _replace_home_with_var(self, path: str) -> str:
        ...
        Returns:
            ...
            Example: '~/dynamo/...' -> '$HOME/dynamo/...'
            Example: '~/dynamo/a:~/dynamo/b' -> '$HOME/dynamo/a:$HOME/dynamo/b'
        """
    ...
    def _print_system_info(self) -> bool:
        ...
        try:
            self._gpu_error = any(isinstance(line, str) and line.startswith("❌") for line in gpu_lines)
            if self._gpu_error:
                system_errors_found = True
        except Exception:
            pass
   ...
   ({len(paths)} workspace component paths found)"""
    ...
    def _show_build_options(self, display_pythonpath: Optional[str] = None):
    ...
    # show_usage_examples merged into _show_build_options
