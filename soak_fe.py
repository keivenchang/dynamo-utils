...
class SharedModelState:
    ...
    def __init__(self):
        ...
        self.refetch_requested_event = (
            asyncio.Event()
        )  # Signal to request model re-fetch
...
class ModelFetcher:
    ...
    async def invalidate_models(self):
        ...
        print(
            "ModelFetcher: Model information invalidated, re-fetch requested",
            file=sys.stderr,
        )

    async def ensure_model_available(
        self, session: aiohttp.ClientSession, retry_interval: float = 2.0
    ) -> bool:
        ...
        while not self.cancelled:
            ...
            async with self.shared_state.lock:
                if success:
                    ...
                    if retry_count > 0:
                        print(
                            f"ModelFetcher: Models found after {retry_count} retries: {self.shared_state.model_names}",
                            file=sys.stderr,
                        )
                    else:
                        print(
                            f"ModelFetcher: Models found: {self.shared_state.model_names}",
                            file=sys.stderr,
                        )
                    return True
                else:
            ...
            if error_msg != last_error_msg or retry_count % 5 == 1:
                print(
                    f"ModelFetcher: {error_msg}, retrying in {retry_interval} seconds... (attempt {retry_count})",
                    file=sys.stderr,
                )
...
class LoadTestWorker:
    ...
    async def run(self, session: aiohttp.ClientSession) -> None:
        ...
        print(
            f"Worker {self.worker_id}: Model available, starting to blast queries...",
            file=sys.stderr,
        )
            ...
            try:
                if self.config.streaming:
                    (
                        ...
                        status_code,
                        content,
                    ) = await self._make_streaming_request(session, request_id)
                else:
                    (
                        success,
                        duration,
                        status_code,
                        content,
                    ) = await self._make_non_streaming_request(session, request_id)
                ...
                if not success:
                    print(
                        f"Worker {self.worker_id}: HTTP failure during queries, requesting model re-fetch",
                        file=sys.stderr,
                    )
                    ...
                    if self.cancelled:
                        break
                    print(
                        f"Worker {self.worker_id}: Model available again, resuming queries...",
                        file=sys.stderr,
                    )
                ...
                print(
                    f"Worker {self.worker_id} request {request_id} failed: {e}",
                    file=sys.stderr,
                )
                self.stats.add_result(self.worker_id, request_id, False, 0.0, 0, "")
                # Exception occurred, go back to model check
                print(
                    f"Worker {self.worker_id}: Exception during queries, requesting model re-fetch",
                    file=sys.stderr,
                )
                # Request model re-fetch through ModelFetcher
                await self.model_fetcher.invalidate_models()
    ...
    async def _wait_for_model_availability(self) -> None:
        ...
        while not self.cancelled:
            async with self.shared_state.lock:
                if self.shared_state.model_available:
                    print(
                        f"Worker {self.worker_id}: Model available: {self.shared_state.model_names}",
                        file=sys.stderr,
                    )
                    return
            ...
            print(
                f"Worker {self.worker_id}: Waiting for model availability...",
                file=sys.stderr,
            )
            try:
                await asyncio.wait_for(
                    self.shared_state.model_ready_event.wait(), timeout=2.0
                )
                if self.shared_state.model_available:
                    print(
                        f"Worker {self.worker_id}: Model became available",
                        file=sys.stderr,
                    )
                    return
            except asyncio.TimeoutError:
                print(
                    f"Worker {self.worker_id}: Model not available, retrying in 2 seconds...",
                    file=sys.stderr,
                )
                continue
        ...
        while not self.cancelled:
            ...
            try:
                ...
                if any(
                    ...
                    print(
                        f"Worker {self.worker_id}: Connection error (attempt {attempt}): {e}. Retrying in {wait_time}s...",
                        file=sys.stderr,
                    )
                    await asyncio.sleep(wait_time)
                    continue
                    ...
                    print(
                        f"Worker {self.worker_id}: Non-connection error: {e}",
                        file=sys.stderr,
                    )
                    return False, 0.0, 0, ""
    ...
    def _build_payload_and_url(self, prompt: str, streaming: bool) -> Tuple[Dict, str]:
        ...
        if model_name is None and self.shared_state.model_names:
            ...
            model_name = (
                "unknown"  # Fallback, should not happen if model check succeeded
            )

        # Check if this is a LLaVA model (multimodal) - force v1_completion for these
        is_llava = "llava" in model_name.lower()

        if self.config.version == "v1_completion" or is_llava:
            # Use v1/completions endpoint with prompt-based payload
            # This is the proper format for LLaVA multimodal models
            payload = {
                "model": model_name,
                "prompt": prompt,
                "stream": streaming,
                    ...
                    if choices:
                        ...
                        if content:
                            ...
                            content_parts.append(
                                content
                            )  # Keep original for final response
    ...
    async def _make_streaming_request(
        ...
        # Check if this is a LLaVA model (multimodal) - use appropriate prompt format
        model_name = self.config.model or (
            self.shared_state.model_names[0] if self.shared_state.model_names else ""
        )
        if "llava" in model_name.lower():
            prompt = f"USER: `Tell me 3 knock knock jokes. ({request_id} / {self.worker_id}) ASSISTANT:"
        else:
            prompt = f"Tell me 3 knock knock jokes. ({request_id} / {self.worker_id})"
            ...
            try:
                ...
                async with session.post(url, json=payload, timeout=timeout) as response:
                    ...
                    if response.status == 200:
                        ...
                        print(
                            f"Worker {self.worker_id}: HTTP error {response.status}",
                            file=sys.stderr,
                        )
    ...
    def _print_request_curl_command(self, payload: dict, url: str) -> None:
        ...
        endpoint_name = (
            "chat/completions" if "/chat/completions" in url else "completions"
        )
    ...
    async def run(self) -> None:
        ...
        async with aiohttp.ClientSession(
            ...
            try:
                ...
                model_available = await self.model_fetcher.ensure_model_available(
                    session, self.config.model_fetch_retry_interval
                )
                ...
                if self.config.verbose:
                    print(
                        f"Model information ready. Available models: {self.shared_state.model_names}"
                    )

                # Create a background task to handle model re-fetching when needed
                model_monitor_task = asyncio.create_task(
                    self._monitor_model_availability(session)
                )

                # Create tasks for all workers
                self.tasks = [
                    asyncio.create_task(worker.run(session)) for worker in workers
                ]
                self.tasks.append(
                    model_monitor_task
                )  # Include monitor task for cleanup

                # Monitor progress - update stats start time to actual test start
                start_time = time.time()
                self.stats.start_time = (
                    start_time  # Update stats to use actual test start time
                )
                last_report = 0

                while (
                    self.running and time.time() - start_time < self.config.duration_sec
                ):
                    ...
                    if int(current_time) >= last_report + 5:
                        ...
                        chars_per_second = (
                            total_chars / current_time if current_time > 0 else 0
                        )

                        print(
                            f"Elapsed: {int(current_time)}s, Active workers: {active_workers}, "
    ...
    async def _monitor_model_availability(self, session: aiohttp.ClientSession) -> None:
        ...
        while self.running and not self.interrupted:
            try:
                ...
                await self.model_fetcher.ensure_model_available(
                    session, self.config.model_fetch_retry_interval
                )
                ...
                for result in self.stats.results:
                    ...
                    if self.config.output_responses and result["response_content"]:
                        ...
                        if self.config.workers > 1 and len(content) > 70:
                            ...
                            content = (
                                content[:70]
                                + f"... ({len(original_cleaned) - 70} more chars)"
                            )
...
Examples:
  ...
  %(prog)s --version v1_completion --workers 10  # Use v1/completions endpoint
    ...
    parser.add_argument(
        "--version",
        choices=["v1_chat_completion", "v1_completion"],
    ...
    parser.add_argument(
        "--output", action="store_true", help="Show server responses in output"
    )
    parser.add_argument(
        "--verbose",
        action="store_true",
