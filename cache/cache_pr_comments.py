"""Cache for PR review comments.

Caching strategy:
  - Key: <owner>/<repo>#<pr_number>
  - Value: {ts, comments: [list of comment dicts from GitHub API]}
  - TTL: Short (comments change frequently, but caching helps repeated runs)
  - ETag: EXCELLENT candidate - comments rarely change between runs, 304 saves data transfer
  
NOTE: This stores raw GitHub API comment responses.
"""
...
class PRCommentsCache(BaseDiskCache):
    ...
    def get_if_fresh(self, key: str, ttl_s: int) -> Optional[Dict[str, Any]]:
        ...
        Returns:
            Dict with 'comments' and optionally 'etag' keys, or None if not cached/stale
        """
        with self._mu:
            self._load_once()
