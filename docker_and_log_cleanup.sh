...
#   0 2 * * * NVIDIA_HOME=$HOME/nvidia $HOME/nvidia/dynamo-utils/cron_log.sh docker_and_log_cleanup $HOME/nvidia/dynamo-utils/docker_and_log_cleanup.sh
...
LOCK_FILE="/tmp/dynamo-utils.docker_and_log_cleanup.${USER}.lock"
exec 9>"$LOCK_FILE"
if ! flock -n 9; then
  echo "Another docker_and_log_cleanup.sh is already running; exiting." >&2
  exit 0
fi

echo "[$(date '+%Y-%m-%d %H:%M:%S')] docker_and_log_cleanup.sh starting"
...
echo "[$(date '+%Y-%m-%d %H:%M:%S')] docker_and_log_cleanup.sh done"
