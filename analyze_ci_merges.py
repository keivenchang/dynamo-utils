...
def _render_summary(kind: str, entries_by_sha: dict, page_paths: dict) -> str:
    ...
    for rank, (test_id, n) in enumerate(ranked, 1):
        test_rows.append(
            ...
            f"<td style='white-space:nowrap;'>{_history_bar(test_id)}</td>"
            f"</tr>"
        )
    if not test_rows:
        test_rows.append(
...
"""
    ...
    return "\n".join([
        ...
        "<th>Test</th>"
        "<th title='per-cell: PR / SHA / author / time / status — colored cells link to the job log'>"
        "PR history &mdash; <small style='font-weight:400;color:#586069;'>oldest &#8594; newest</small>"
        "</th></tr></thead>",
        "<tbody>",
        *test_rows,
        "</tbody></table>",
